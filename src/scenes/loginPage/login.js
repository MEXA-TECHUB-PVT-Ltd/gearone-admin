        ...
        .then((response) => {
          ...
          if (response.status == true) {
            ...
            localStorage.setItem(
              "adminemail",
              JSON.stringify(response.result[0].email)
            );
            localStorage.setItem("adminname", "admin");
            localStorage.setItem("adminimageurl", "admin");
            localStorage.setItem(
              "adminID",
              JSON.stringify(response.result[0].id)
            );
            localStorage.setItem("password", JSON.stringify(password));
            localStorage.setItem(
              "two_factor",
              JSON.stringify(response.result[0].two_factor)
            );
            navigate("/ManageUsers");
            setIsloading(false);
          } else {
    ...
    <>
      <Grid container spacig={0}>
        <Grid xs={12} md={6} lg={6} xl={6} align="">
          <Container>
            <Box
              sx={{ pt: { xs: 0, sm: 0, md: 0, lg: 7, xl: 7 } }}
              p={{ lg: 8, xl: 13 }}
            >
                  ...
                  >
                    <Stack direction="column" spacing={0} pt={4}>
                      <Typography
                        ...
                        pb={1}
                        fontSize="12px"
                        sx={{ font: "normal normal normal 17px/26px Roboto" }}
                        color="#1F1F1F"
                      >
                        Email Address
                      </Typography>
